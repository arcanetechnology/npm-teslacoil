...
ALTER TABLE transactions
    ...
    ADD CONSTRAINT transactions_must_either_be_onchain_or_offchain CHECK
        (
            (
                    (address IS NOT NULL) -- if address or txid is defined
                    AND
            ...
            (
                    ...
                    AND
                    (address IS NULL) -- address and txid have to be undefined
                )
        ),

    ADD CONSTRAINT transactions_positive_amount_milli_sat CHECK
        (amount_milli_sat >= 0);

...
func Serve() cli.Command {
	serve := cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if os.Getenv(gin.EnvGinMode) == gin.ReleaseMode {
				...
				tlsKeyFile := c.String("tls-privkey-file")

				log.WithFields(logrus.Fields{
					"tls-cert-file":    tlsCertFile,
					"tls-privkey-file": tlsKeyFile,
	...
	baseFlags := []cli.Flag{
		...
		cli.StringFlag{
			...
			Usage:     "Path to TLS cert.pem file",
			TakesFile: true,
			Required:  os.Getenv(gin.EnvGinMode) == gin.ReleaseMode,
		},
		cli.StringFlag{
			Name:     "tls-privkey-file",
			EnvVar:   "TESLACOIL_TLS_PRIVKEY_FILE",
			Usage:    "Path to TLS privkey.pem file",
			Required: os.Getenv(gin.EnvGinMode) == gin.ReleaseMode,
		},
